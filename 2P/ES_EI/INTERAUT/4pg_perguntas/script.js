...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (pontuacaoSalva) {
        document.getElementById('pontuacao').textContent = `🏆 ${pontuacaoSalva}`;
    }
});

document.addEventListener("DOMContentLoaded", function () {
    // Obtém a pontuação salva no localStorage
    const playerScore = localStorage.getItem("pontuacao") || 0;

    // Atualiza o conteúdo do span com a pontuação
    document.getElementById("score").textContent = playerScore;
});

// Reiniciar o jogo (limpa pontuação e redireciona para a primeira página)
document.getElementById("restart").addEventListener("click", () => {
    localStorage.removeItem("pontuacao"); // Reseta apenas a pontuação
    window.location.href = "../3pg_fases/index.html"; // Redireciona para a página inicial
});

function reproduzirAudio() {
    var audio = document.getElementById('audio');  // Pega o elemento de áudio
    audio.play();  // Reproduz o áudio
}

document.getElementById("homeButtom").addEventListener("click", () => {
